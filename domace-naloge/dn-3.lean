...
theorem vsota_lihih_kvadrat : (n : Nat) → vsota_lihih n = n * n :=
  by
    ...
    | zero => rfl
    | succ n ih => calc
        vsota_lihih (Nat.succ n)
        _ = 2 * n + 1 + vsota_lihih n := rfl
        _ = 2 * n + 1 + n * n := by rw [ih]
        _ = n * n + n + (n + 1) := by omega
        _ = n * (n + 1) + (n + 1) := by rw [Nat.mul_add, Nat.mul_one]
        _ = (n + 1) * n + (n + 1) * 1 := by rw [Nat.mul_comm, Nat.mul_one]
...
def vsota_produktov : Nat → Nat
  | 0 => 0
  | Nat.succ n => vsota_produktov n + (n + 1) * (n + 2)
...
theorem formula_produktov : (n : Nat) → 3 * vsota_produktov n = n * (n + 1) * (n + 2) :=
  by
    ...
    | zero => rfl
    | succ n ih => calc
        3 * vsota_produktov (Nat.succ n)
        _ = 3 * (vsota_produktov n + (n + 1) * (n + 2)) := rfl
        _ = 3 * vsota_produktov n + 3 * ((n + 1) * (n + 2)) := by omega
        _ = n * (n + 1) * (n + 2) + 3 * ((n + 1) * (n + 2)) := by rw [ih]
        _ = (n + 1) * (n + 2) * n + 3 * ((n + 1) * (n + 2)) := by rw [Nat.mul_assoc, Nat.mul_comm]
        _ = ((n + 1) * (n + 2)) * n + ((n + 1) * (n + 2)) * 3 := by omega
        _ = (n + 1) * (n + 2) * (n + 3) := by rw [← Nat.mul_add]

theorem prava_formula_produktov : (n : Nat) → vsota_produktov n = (n * (n + 1) * (n + 2)) / 3 :=
  by
    intro n
    have h := formula_produktov n
    rw [← h]; omega
...
def obrni : {A : Type} → {m : Nat} → Vektor A m → Vektor A m
  | _, _, .prazen => .prazen
  | _, _, .sestavljen x xs => stakni (obrni xs) (.sestavljen x .prazen)

def preslikaj : {A B : Type} → {n : Nat} → (A → B) → Vektor A n → Vektor B n
  | _, _, _, _, .prazen => .prazen
  | _, _, _, f, .sestavljen x xs => .sestavljen (f x) (preslikaj f xs)

def zip : {A B : Type} → {n : Nat} → Vektor A n → Vektor B n → Vektor (A × B) n
  | _, _, _, .prazen, .prazen => .prazen
  | _, _, _, .sestavljen x xs, .sestavljen y ys =>
...
theorem preslikaj_identiteto : {A : Type} → {n : Nat} → (xs : Vektor A n) →
  ...
  by
    ...
    induction xs with
    | prazen => rfl
    | sestavljen x xs' ih => simp [preslikaj, ih]
...
theorem preslikaj_in_zip_se_ujemata : {A B C : Type} → {n : Nat} →
  ...
  by
    ...
    induction xs with
    | prazen => rfl
    | sestavljen x xs' ih => simp [preslikaj, zip, ih]
...
theorem trojna_negacija {P : Prop} : ¬¬¬P ↔ ¬P :=
  by
    ...
    · intro nnnp p
      apply nnnp
      exact dvojna_negacija.mpr p
    · intro np nnp
      apply np
      exact dvojna_negacija.mp nnp
...
theorem paradoks_pivca :
  ...
  by
    ...
    | inl h => exact ⟨g, by intro _; exact h⟩
    | inr h =>
      have h' : ∃ x, ¬ P x := Classical.not_forall.mp h
      obtain ⟨x, hx⟩ := h'
      exact ⟨x, by intro nhx; exact False.elim (hx nhx)⟩
...
theorem zrcaljenje_ohrani_visino :
  ...
  by
    ...
    | sestavljeno x l d ihl ihd =>
      simp [zrcali, visina, ihl, ihd]; omega
...
theorem pomozna_lema : ∀ (t : Drevo Nat) (acc : Nat), vsota'.aux t acc = vsota t + acc :=
  by
    intro t acc
    induction t generalizing acc with
    | prazno => simp [vsota'.aux, vsota]
    | sestavljeno x l d ihl ihd =>
      simp [vsota'.aux, vsota, ihd, ihl]; omega
...
theorem vsota_eq_vsota' : ∀ {t : Drevo Nat}, vsota t = vsota' t :=
  by
    ...
    | prazno => rfl
    | sestavljeno x l d ihl ihd =>
      simp [vsota, vsota', vsota'.aux, pomozna_lema]; omega
